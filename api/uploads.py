...
async def upload_session_file(
    ...
    upload = await create_upload(db, user.id, session_id, file)
...
async def get_session_uploads(
    ...
    uploads = await list_session_uploads(db, user.id, session_id)
    return UploadListResponse(items=uploads)


@router.get("/{upload_id}", response_model=UploadResponse)
async def get_upload(upload_id: UUID, user: CurrentUser, db: DatabaseSession) -> UploadResponse:
    upload = await get_owned_upload(db, user.id, upload_id)
    return UploadResponse.model_validate(upload)


@router.post("/{upload_id}/download", response_model=UploadDownloadResponse)
async def download_upload(
    upload_id: UUID, user: CurrentUser, db: DatabaseSession
) -> UploadDownloadResponse:
    url, expires_at = await create_upload_download(db, user.id, upload_id)
    return UploadDownloadResponse(url=url, expires_at=expires_at)


@router.delete("/{upload_id}", status_code=status.HTTP_204_NO_CONTENT)
async def remove_upload(upload_id: UUID, user: CurrentUser, db: DatabaseSession) -> Response:
    await delete_upload(db, user.id, upload_id)
